  ...
  <div>
    ...
    <main class="flex-1 w-full md:w-4/5 shadow-sm mx-auto">
      <div class="w-full shadow-sm mx-auto">
              ...
              >
                ...
                <p class="pt-2">
                  <span
                    ...
                    class="text-red-600 text-2xl font-bold uppercase"
                  >
                    Hết hàng
                  </span>
                  <span
                  ...
                  <span v-else class="text-red-600 text-2xl font-bold"
                    >Liên Hệ</span
                  >
                </p>
              </router-link>
        ...
        <div
          v-if="!loading && !isFiltered && displayProducts.length === 0"
          class="text-center py-8"
        >
          <p class="text-gray-500 text-xl">
...
export default {
  ...
  methods: {
    ...
    handleFilteredProducts(filteredArray) {
      ...
      this.isFiltered =
        filteredArray !== this.originalProducts &&
    ...
    async loadProduct() {
      try {
        this.loading = true;
        console.log(
          `Loading page ${this.currentPage} for category ${this.categoryId}`
        );
        ...
        console.log(
          `Loaded ${this.products.length} products, total: ${this.totalItems}`
        );
      } catch (error) {
    ...
    async loadAllProducts() {
      try {
        ...
        console.log(
          `Loaded ${this.originalProducts.length} total products for filtering`
        );
      } catch (error) {
      ...
      async handler(newVal, oldVal) {
        if (newVal && newVal !== oldVal) {
          ...
          try {
            ...
            await Promise.all([this.loadProduct(), this.loadAllProducts()]);
          } catch (error) {

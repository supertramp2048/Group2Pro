<template>
  <div>
    ...
    <div class="w-full md:w-10/12 shadow-sm mx-auto">
      ...
      <div v-if="isFiltered && displayProducts.length === 0" class="text-center py-8">
        <p class="text-gray-500 text-xl">Không tìm thấy sản phẩm nào phù hợp với bộ lọc</p>
      </div>
      ...
      <div class="grid grid-cols-4">
        <div v-for="product in displayProducts" :key="product.id" class="mt-3.5">
          <div
    ...
    <div v-if="!isFiltered" class="flex justify-center space-x-2 mt-6 sticky bottom-0">
...
export default {
  ...
  methods: {
    ...
    async loadProduct() {
      let res = await fetch(
        `http://localhost:3000/posts?categoryId=${this.categoryId}&_page=${this.currentPage}&_limit=20`
      );
    ...
    async loadAllProducts() {
      let res = await fetch(
        `http://localhost:3000/posts?categoryId=${this.categoryId}`
      );
